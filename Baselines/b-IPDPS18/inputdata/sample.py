...
def randomSample(inputfile, outputfile, opt, ratio):
    ...
    with open(inputfile, 'rb') as fin:
        ...
        samplepoints = array.array('d')
        if opt == 'point':
            sampleidx = random.sample(range(len(a)), int(sample_elem_num))
            for i in sorted(sampleidx):
                samplepoints.append(a[i])
        if opt == 'block':
            sampleidx = random.sample(range(int(len(a)/4)), int(sample_elem_num/4))
            for i in sorted(sampleidx):
                samplepoints += a[i*4:(i+1)*4]
        if opt == 'chunk':
            sampleidx = random.sample(range(int(len(a)/chunksize)), int(sample_elem_num/chunksize))
            for i in sorted(sampleidx):
...
def main():
    ...
    if args['sample'] == 'interval':
        randomSample(args['input'], args['randopt'] + 'intervalSample/' + args['input'] + '.sample', args['randopt'], sample_ratio)
    if args['sample'] == 'random':
        randomSample(args['input'], args['randopt'] + 'randomSample/' + args['input'] + '.sample', args['randopt'], sample_ratio)
