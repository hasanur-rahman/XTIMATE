...
int dataLength, int *outSize, double min, double max)
{
	...
	unsigned char* pwrErrBoundBytes = (unsigned char*)malloc(pwrErrBoundBytes_size);
        ...
        printf("\t compute_segment_precisions_double_1D: \n");	
        printf("\t\t oriData[0-4] = %lf, %lf, %lf, %lf, %lf , dataLength:  \n", oriData[0], oriData[1], oriData[2], oriData[3], oriData[4], dataLength);	
	...
	computeReqLength_double(realPrecision, radExpo, &reqLength, &medianValue);	
        printf("\t computeReqLength_double: \n");	
        printf("\t\t realPrecision %lf, radExpo %d, reqLength %d, medianValue %lf: \n", realPrecision, radExpo, reqLength, medianValue);	
	...
	{
            printf("type[%d:%d] = %d, %d, %d, %d, %d \n", i, i+4, type[i], type[i+1], type[i+2], type[i+3], type[i+4]);
        }	
        ...
        for (int i = 1; i < dataLength; i++){
            ...
            ttl += type[i];
            cnt++;
            if (type[i] < m1) m1 = type[i];
            if (type[i] > m2) m2 = type[i];
        }
        ...
        for (int i = 1; i < dataLength; i++){
            if (type[i] == 0) continue;
            printf("type[%d], %d\n", i, type[i]); // Hasan
            var = var + (type[i] - avg) * (type[i] - avg);
        }
        printf("type max = %d, min = %d, range = %d, avg = %d, std variance = %lu \n", m2, m1, m2 - m1, ttl/cnt, var/cnt);
...
//	int expSegmentsInBytes_size = convertESCToBytes(esc, &expSegmentsInBytes);
	...
	TightDataPointStorageD* tdps;
        ...
        printf("\t new_TightDataPointStorageD\n");	
		// Hasan
