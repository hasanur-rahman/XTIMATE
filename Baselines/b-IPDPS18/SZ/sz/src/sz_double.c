...
int dataLength, double realPrecision, double valueRangeSize, double medianValue_d)
{
	...
	short reqExpo = getPrecisionReqLength_double((double)realPrecision); /*decided by the error bound parameters*/
	short radExpo = getExponent_double(valueRangeSize/2); /*decided by the value range of input data*/

	computeReqLength_double(realPrecision, radExpo, &reqLength, &medianValue);	
        printf("reqExpo = %d, radExpo = %d, reqLength = %d, medianValue = %lf\n", reqExpo, radExpo, reqLength, medianValue);
    ...
    //     printf("dataLength = %d, exactDataNum = %d\n", dataLength, exactDataNum);
	int exactDataNum = exactLeadNumArray->size;
	printf("dataLength = %d, exactDataNum = %d, curve fitting hit ratio = %f \n", dataLength, exactLeadNumArray->size, 1.0 - 1.0 * exactLeadNumArray->size / dataLength);
	...
	printf("exactDataNum=%d, exactMidByteArray->size=%d,resiBitLengthArray->size=%d\n", 
			exactDataNum, exactMidByteArray->size, resiBitLengthArray->size);
...
{
	SZ_Reset();
        printf("\t\t\t SZ_compress_double_1D_MDQ parameters: \n");
        printf("\t\t\t oriData[0:5]=[%lf, %lf, %lf, %lf, %lf], dataLength=%d, realPrecision=%lf, valueRangeSize=%lf, medianValue_d=%lf \n", oriData[0], oriData[1], oriData[2], oriData[3], oriData[4], dataLength, realPrecision, valueRangeSize, medianValue_d );
...
int errBoundMode, double absErr_Bound, double relBoundRatio)
{
	...
	if(valueRangeSize <= realPrecision)
	{
                printf("\n SZ_compress_args_double_withinRange:\n ");
                printf("\n\n newByteData = %s, oriData =%lf , dataLength= %d, outSize =%d:\n", *newByteData, *oriData, dataLength, outSize );
		SZ_compress_args_double_withinRange(newByteData, oriData, dataLength, outSize);
	}
	else
	{
		...
		{
			if(errBoundMode==PW_REL){
                                printf("\t SZ_compress_args_double_NoCkRngeNoGzip_1D_pwr:\n");
                                printf("\t\t &tmpByteData is a char* address, oriData[0:5]=[%lf, %lf, %lf, %lf, %lf], r1=%d, realPrecision=%lf, tmpOutSize=%d, valueRangeSize=%lf, medianValue=%lf \n", oriData[0], oriData[1], oriData[2], oriData[3], oriData[4], r1, realPrecision, tmpOutSize, valueRangeSize, medianValue );
		...
		{
                        printf("further gzip compression triggered\n");
                        printf("before gzip the byte size is %d\n", tmpOutSize);
			*outSize = (int)zlib_compress2(tmpByteData, tmpOutSize, newByteData, gzipMode);
                        printf("after gzip the byte size is %d\n", *outSize);
			free(tmpByteData);			
		}
		else
		{
	...
	*reqLength = 12+radExpo - reqExpo; //radExpo-reqExpo == reqMantiLength
	...
	if(*reqLength>64)
	{
		...
		*medianValue = 0;
        ...
        printf("reqLength(%d) = 12 + radExpo(%d) - reqExpo(%d)\n", *reqLength, radExpo, reqExpo);
}
