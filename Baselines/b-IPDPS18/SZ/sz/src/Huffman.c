...
void printnode(node n){
    if (n){
        printf("n->freq = %d, n->c=%d, n->t = %d\n", n->freq, n->c, n->t);
    }

    if (n->left){
        printf("left->freq = %d, left->c=%d\n", n->left->freq, n->left->c);
    }
    if (n->right){
        printf("right->freq = %d, right->c=%d\n", n->right->freq, n->right->c);
    }
}
...
{
        printnode(n);
        printf("\t node n, int len = %d, unsigned long out1 = %lu, unsigned long out2 = %lu\n", len, out1, out2);  
    printf("build code:\n");  
	if (n->t) {
		...
		else
		{
			...
			(code[n->c])[1] = out2 << (128 - len);
                ...
                printnode(n);
                printf("\t node n, int len = %d, unsigned long out1 = %lu, unsigned long out2 = %lu, n->c = %d, code[n->c])[0] = %lu, code[n->c])[1] = %lu\n", len, out1, out2, n->c, code[n->c][0], code[n->c][1]);  
	...
	if(index == 0)
	{
		out1 = out1 << 1;
		out1 = out1 | 0;
                printf("out1 for left child = %lu\n", out1);
		build_code(n->left, len + 1, out1, 0);
		out1 = out1 | 1;
                printf("out1 for right child = %lu\n", out1);
...
void encode(int *s, int length, unsigned char *out, int *outSize)
{
	...
	for (i = 0;i<length;i++) 
	{
		...
		printf("i = %d, state = %d : bitsize = %d, code[state][0] >> (64-cout[state]) = %u\n",i, state, bitSize, (code[state])[0] >> (64-cout[state])); 
...
void encode_withTree(int *s, int length, unsigned char **out, int *outSize)
{
	...
	for (i = 0; i < stateNum; i++){
        ...
        printf("nodeCount %d\n", nodeCount);
	nodeCount = nodeCount*2-1; //huffman coding node number
	...
	*outSize = 4+treeByteSize+enCodeSize;
	...
	printf("outsize = 4 + treeByteSize=%d + enCodeSize=%d bytes\n", treeByteSize, enCodeSize);
