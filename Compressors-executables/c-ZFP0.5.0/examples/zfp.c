...
int main(int argc, char* argv[])
{
  ...
  */
  ...
  if (inpath) {
    ...
    /*
    if (type == zfp_type_float) dataFloat = (float*)fi; 
    else if (type == zfp_type_double) dataDouble = (double*)fi;
    else {printf("data type should be either float or double!"); exit(0);}
    */
    ...
    switch (mode) {
      case 'a':
        if (type == zfp_type_float){
          float mn_temp = ((float*)fi)[0], mx_temp = ((float*)fi)[0];
          for (it = 1; it < nbEle; it++) {
            if(mn_temp > ((float*)fi)[it]) mn_temp = ((float*)fi)[it];
            if(mx_temp < ((float*)fi)[it]) mx_temp = ((float*)fi)[it];
          }
          float range = mx_temp - mn_temp;
          real_tolerance = tolerance * range;
          fprintf(stderr, "value range %.8f, real tolerance = %.8lf\n", range, real_tolerance);
        }
        else if (type == zfp_type_double){
          double mn_temp = ((double*)fi)[0], mx_temp = ((double*)fi)[0];
          for (it = 1; it < nbEle; it++) {
            if(mn_temp > ((double*)fi)[it]) mn_temp = ((double*)fi)[it];
            if(mx_temp < ((double*)fi)[it]) mx_temp = ((double*)fi)[it];
          }
          double range = mx_temp - mn_temp;
          real_tolerance = tolerance * range;
          fprintf(stderr, "value range %.8lf, real tolerance = %.8lf\n", range, real_tolerance);
        }
        else{
          fprintf(stderr, "data type should be either float or double!");
          exit(0);
        }
        zfp_stream_set_accuracy(zfp, real_tolerance, type);
        break;
      case 'p':
  ...
  if (!quiet) {
    ...
    fprintf(stderr, " raw=%lu zfp=%lu compressionRatio=%.3g rate=%.4g", (unsigned long)rawsize, (unsigned long)zfpsize, (double)rawsize / zfpsize, CHAR_BIT * (double)zfpsize / (nx * ny * nz));
    if (stats)
